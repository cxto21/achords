...
def register_agent():
    ...
    # Check .achords exists
    achords_dir = Path('.achords')
    ...
    # agent_id
    while True:
        agent_id = input("Agent ID [e.g., agent-a-001]: ").strip()
        if not agent_id:
    ...
    while True:
        agent_type = input(f"Agent type {valid_types} [ai]: ").strip().lower()
        if not agent_type:
            agent_type = 'ai'
    ...
    registry_data['last_updated'] = datetime.utcnow().isoformat() + 'Z'
